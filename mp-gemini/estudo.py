...
def compilar_arquivos(
    ...
    arquivo_saida,
    ignorar_pastas=None,
    ignorar_arquivos=None,
    ignorar_extensoes=None,
):
    """
    Compila arquivos ignorando pastas, nomes específicos e extensões.
    """
    if ignorar_pastas is None:
        ignorar_pastas = {"venv", ".git", "__pycache__"}
    if ignorar_arquivos is None:
        ignorar_arquivos = {".env", "conteudo_total.txt"}
    if ignorar_extensoes is None:
        ignorar_extensoes = {".csv", ".pyc", ".exe"}
...
if __name__ == "__main__":
    compilar_arquivos(pasta_raiz=".", arquivo_saida="conteudo_total.txt")
